...
# 3. 특정 메시지 즉시 전달 (단일 요청)
python test.py "오늘 서울 날씨 알려줘"
```
...
| **실시간 검색** | "오늘 서울 우면동 날씨가 어때?" | `Search Worker`가 Google Search를 통해 최신 정보 제공 |
| **기기 제어** | "거실 TV 켜줘" / "볼륨 높여줘" | `Device Worker`가 SDB 명령 후 결과 텍스트 및 결과 UI(A2UI) 반환 |
| **UI 디자인** | "영화 예약 화면 하나 그려줘" | `A2UI Worker`가 도구 없이 A2UI v0.9 규격의 JSON 코드 생성 |
| **복합 요청** | "안녕? 에어컨 켜고 오늘 날씨 알려줘" | `Router`가 3개 태스크로 분류 후 각 워커의 결과를 통합하여 응답 |

## 라이선스
MIT License

---
**마지막 수정 날짜:** 2026-03-20
