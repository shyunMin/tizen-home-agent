...
2. **2단계 (Worker)**: 분류된 태스크에 따라 각 분야의 **전담 워커(Worker)**들이 병렬로 실행됩니다.
    ...
    - **Web Search Agent**: **Google Search Grounding**을 사용하여 최신 정보를 검색하고, 의도에 맞는 최적의 URL을 기기의 `app_launcher`로 실행하여 화면에 표시합니다.
...
---
**마지막 수정 날짜:** 2026-03-24 18:34

**수정 사항:** 아키텍처 다이어그램 및 에이전트 명칭을 최신 반영 사항에 맞게 추가 업데이트하였습니다. (앱 생성 에이전트 제거, 검색/프레젠터 -> 웹 검색 에이전트)
