...
graph TD
    ...
    router_node -->|briefing| briefing_worker["📰 Briefing Worker"]
    router_node -->|app_deploy| app_deploy_worker["📦 App Deploy Worker"]
    ...
    briefing_worker --> reconstructor
    app_deploy_worker --> reconstructor
...
2. **2단계 (Worker)**: 분류된 태스크에 따라 각 분야의 **전담 워커(Worker)**들이 병렬로 실행됩니다.
    ...
    - **App Deploy Worker**: 사용자의 요청을 바탕으로 외부 API 서버에 코드를 요청하고, 결과물을 Tizen 기기에 원격 배포 및 압축 해제합니다.
...
| **앱 실시간 배포** | "날씨 위젯 앱 만들어서 설치해줘" | `App Deploy Worker`가 API 호출로 코드를 다운받아 SDB를 통해 Tizen 기기에 전송하고 압축 해제하여 배포 완료 |

## 라이선스
MIT License

---
**마지막 수정 날짜:** 2026-03-23 19:40
