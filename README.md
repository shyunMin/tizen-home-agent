...
---
**마지막 수정 날짜:** 2026-03-25 10:55

**수정 사항:** 
1. **화면 분석 자동화**: `vision` 태스크가 정상 분류되도록 `config.py`의 `TaskType`을 업데이트하고, 화면 설명 요청 시 자동으로 스크린샷을 찍어 분석하도록 로직을 수정했습니다.
2. **유튜브 추천 라우팅 개선**: 유튜브 추천 요청이 영상 플레이어(`youtube_play`)가 아닌 카드 뉴스(`briefing`)로 정확히 분류되도록 가이드라인을 강화했습니다.
