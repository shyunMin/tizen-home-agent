...
graph TD
    ...
    subgraph Agents ["전문 에이전트 그룹 (Agents)"]
        direction 
        ChatAgent["💬 대화 에이전트"]
        SearchAgent["🔍 검색/프레젠터 에이전트"]
        BriefingAgent["📰 뉴스 브리핑 에이전트"]
        AppGenAgent["📦 앱 생성 에이전트"]
        YouTubeAgent["▶️ 유튜브 플레이 에이전트"]
        GenUIAgent["✨ 고품질 시각화(GenUI) 에이전트"]
        DeviceAgent["📱 기기 제어 에이전트"]
        DesignAgent["🎨 디자인(A2UI) 에이전트"]
    end
    
    Orchestrator -->|일상 대화| ChatAgent
    Orchestrator -->|최신 정보| SearchAgent
    Orchestrator -->|뉴스 요약| BriefingAgent
    Orchestrator -->|앱 제작| AppGenAgent
    Orchestrator -->|영상 재생| YouTubeAgent
    Orchestrator -->|복잡한 시각화| GenUIAgent
    Orchestrator -->|기기 설정| DeviceAgent
    Orchestrator -->|UI 프레임| DesignAgent
    
    ChatAgent --> HTMLSynthesizer
    SearchAgent --> HTMLSynthesizer
    BriefingAgent --> HTMLSynthesizer
    AppGenAgent --> HTMLSynthesizer
    YouTubeAgent --> HTMLSynthesizer
    GenUIAgent --> HTMLSynthesizer
    DeviceAgent --> HTMLSynthesizer
    DesignAgent --> HTMLSynthesizer
    
    HTMLSynthesizer["🔄 HTML 결과 통합기 (HTML Synthesizer)\n(결과 수합 및 HTML UI 반환)"] --> END([✅ 최종 응답 반환])
```
...
2. **2단계 (Worker)**: 분류된 태스크에 따라 각 분야의 **전담 워커(Worker)**들이 병렬로 실행됩니다.
    ...
    - **Briefing Worker**: 정보를 기기에 카드 뉴스 형태로 브리핑하는 HTML을 자체 생성 후 배포하여 표시합니다.
    - **App Deploy Worker**: 사용자의 요청을 바탕으로 LLM이 Tizen용 단일 파일 HTML 앱을 직접 생성하고, SDB를 통해 Tizen 기기에 실시간 배포 및 실행합니다.
    - **YouTube Worker**: 사용자의 요청에서 영상 제목을 추출하고 `youtube-search` 패키지로 실제 플레이 가능한 영상 URL을 얻은 후, 전체화면 HTML로 만들어 기기에서 자동 재생시킵니다.
    - **GenUI Worker**: CopilotKit의 `OpenGenerativeUI` MCP 서버를 통해, 복잡한 시각화나 애니메이션, 차트 등의 세련된 단일 HTML 문서를 조립하여 기기에 띄웁니다.
...
sequenceDiagram
    ...
    Server->>Recon: 워커 다중 결과 수합 및 통합 답변/HTML 합성 (HTML Synthesizer)
...
| **앱 실시간 배포** | "날씨 위젯 앱 만들어줘" | `App Deploy Worker`가 직접 HTML 코드를 생성하여 Tizen 기기에 전송하고 즉시 화면에 실행 |
| **유튜브 재생** | "아이유의 좋은날 유튜브에서 재생해줘" | `YouTube Worker`가 영상을 검색해 플레이 가능한 HTML로 만들고 TV에서 자동 재생 |
| **고품질 화면 생성** | "데이터 정렬 알고리즘 시각화해서 보여줘" | `GenUI Worker`가 `OpenGenerativeUI` MCP 서버의 `assemble_document` 도구를 활용해 완벽한 HTML로 조립 후 Tizen 기기에 실행 |

## 라이선스
MIT License

---
**마지막 수정 날짜:** 2026-03-24 15:55

**수정 사항:** 불필요한 JSON 생성 처리기를 제거하고, `ui_code`에 순수 HTML 코드를 담아서 클라이언트로전달할 수 있도록 `html_synthesizer_node`로 리팩토링 하였습니다.
