...
async def router_node(state: AgentState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "5. briefing: 최신 정보를 검색하여 깔끔한 카드 뉴스 형태의 HTML로 브리핑하거나 **다양한 분야(장소, 음식, 영상 등)의 추천**을 수행하는 경우\n"
...
async def briefing_worker_node(state: AgentState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "- 모든 최종 텍스트 콘텐츠(카드 내 설명 등)는 **한국어**로만 작성하여 TV 화면에 깔끔하게 출력되도록 합니다.\n\n"
        "### [수행 단계]\n"
        "1. 정보 검색: 사용자의 요청 사항에 부합하는 최신 정보를 검색하세요. 정보가 부족하면 스스로 합리적 추론을 하여 추천 정보를 3~5개 선정합니다.\n"
