...
async def briefing_worker_node(state: AgentState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "### [중요 지침]\n"
        "- **절대로 사용자에게 추가 정보를 묻지 마세요.** (예: 어느 지역인가요?, 구체적으로 어떤 장르인가요? 등)\n"
        "- 정보가 부족하거나 모호하다면, 현재 시점(오늘 날짜/트렌드)에서 가장 인기 있거나 추천할 만한 정보를 **스스로 판단하여 검색**하고 결과를 즉시 구성하세요.\n"
        "- 사용자가 '추천해줘'와 같이 막연하게 요청하더라도, 검색 엔진을 활용해 최근 핫한 주제나 관련성 높은 정보를 찾아내어 3~5개의 카드로 만드세요.\n\n"
        "### [수행 단계]\n"
